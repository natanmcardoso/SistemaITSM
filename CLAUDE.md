...
- ⏳ **Fase 12 — Catálogo de Serviços + Serviços.** Tabela `services` nova (nome, categoria, descrição, formulário padronizado opcional) — a tela de Configurações cadastra, a aba Catálogo consome pro usuário final escolher ao abrir chamado (em vez de só texto livre).
- ⏳ **Fase 13 — Calendários (SLA por horário comercial).** **Risco alto — reabre lógica já fechada.** Hoje `sla_due_at` é calculado corrido (24/7) a partir de `created_at` (Fase 4, sub-fase SLA). Introduzir calendário/horário comercial muda essa conta pra todos os chamados, não só os novos — exige plano de migração dos SLAs já calculados, não só código novo. Tratar como a fase mais delicada do lote; rodar os testes de SLA já existentes antes e depois pra garantir que nada regrediu.
...
12. ⏳ Catálogo de Serviços + Serviços — tabela `services`, tela de cadastro (Configurações) e tela de catálogo (usuário final) → testar: criar serviço, conferir que aparece no catálogo, abrir chamado a partir de um serviço do catálogo.
13. ⏳ Calendários (SLA por horário comercial) — risco alto, reabre o cálculo de `sla_due_at` já fechado → testar: comparar `sla_due_at` antes/depois da mudança pros mesmos chamados, rodar toda a suíte de SLA já existente.
