...
Próximo passo real: falta 1 lacuna do dashboard do gestor — **resolve-by-user** (usuário fecha o próprio chamado a partir da sugestão da IA → `resolved_by_ai = true` → métrica "% resolvido por IA" no dashboard). Provavelmente precisa de um mínimo de `kb_articles` pra fazer sentido (a sugestão que o usuário estaria "aceitando"). Depois disso, a Fase 4 e as 4 métricas centrais do design doc (§2.3) ficam 100% fechadas; só então faria sentido considerar a Fase 5 (RMM, fora do escopo do MVP atual).
