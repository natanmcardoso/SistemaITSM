...
- ✅ Fecha os 2 últimos gaps mapeados: acompanhamento do chamado pelo usuário final (§2.1) e busca de KB pelo técnico (§2.2).
  ...
  - **Achado durante a verificação:** a extensão do Claude in Chrome não conectou desta vez (4 tentativas, sem sucesso) — essa fase foi validada por build limpo (`tsc` + `vite build`) e por chamadas reais via `curl` aos endpoints novos (`requester_id`, `query`), não por fluxo completo no navegador como as fases anteriores. Vale um passo de verificação visual manual (`/run` ou o usuário testando) antes de considerar 100% fechado.
