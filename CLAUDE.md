...
- ✅ Fase 4, tela 3/3 — dashboard do gestor (testada: `backend/test_phase4_dashboard.py` passando + verificação visual via browser automation). Duas decisões levantadas e confirmadas com o usuário antes de codar:
  ...
  - **Achado não corrigido nesta fase:** `test_phase1_data_model.py` está quebrado — sua limpeza faz `DELETE FROM categories WHERE name = 'Rede'`, o que colide com a categoria real "Rede" que o `seed_dev_data.py` deixa persistida no banco (violação de FK, pois tickets seedados referenciam essa categoria). Bug pré-existente, descoberto ao rodar a suíte completa nesta fase; não bloqueia nada da Fase 4, mas o teste da Fase 1 não passa mais como está.

Próximo passo real: a Fase 4 (frontend) está completa — as 3 telas (fila, novo chamado, dashboard) foram implementadas e testadas. Falta decidir o que vem depois: corrigir o `test_phase1_data_model.py` quebrado, fechar as lacunas de SLA/resolve-by-user, ou seguir para a Fase 5 (RMM, fora do escopo do MVP atual).
